...
func (p *CodexQuotaProvider) FetchWindows(ctx context.Context, a *auth.Auth, model string) ([]auth.QuotaWindow, error) {
	...
	// NOTE: Codex quota probing relies on the ChatGPT web client backend, not the public OpenAI API.
	req, err := http.NewRequestWithContext(ctx, "GET", "https://chatgpt.com/backend-api/wham/usage", nil)
	if err != nil {
		return nil, fmt.Errorf("codex quota request build failed: %w", err)
	}
